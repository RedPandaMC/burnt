"""GCP Databricks PricingBackend — live GCE prices + DBU rates."""
...
PHOTON_MULTIPLIER_GCP = Decimal("2.5")
...
class GcpDatabricksBackend:
    """PricingBackend for Databricks on GCP.

    GCE rates: GCP Cloud Billing Catalog API (public, no auth, 24 h cache).
    ...
    def map(self, graph: CostGraph) -> object:
        """Map CostGraph to CostEstimate using GCP GCE + DBU rates."""
        ...
        try:
            from ._prices import get_gce_price_usd

            gce_price_per_hour = get_gce_price_usd(self._machine_type, self._region)
        except Exception as exc:
            _log.warning(
                "Could not fetch GCE price for %s: %s", self._machine_type, exc
            )
            gce_price_per_hour = Decimal("0")
        ...
        for node in graph.nodes:
            ...
            run_hours = data_gb / Decimal("3600")

            dbu_per_hour = Decimal("1")
            if node.photon_eligible:
                dbu_per_hour = dbu_per_hour * PHOTON_MULTIPLIER_GCP

            node_dbu_cost = run_hours * dbu_per_hour * sku_rate
            node_gce_cost = run_hours * gce_price_per_hour
            node_total = node_dbu_cost + node_gce_cost
