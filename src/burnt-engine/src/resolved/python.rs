...
fn py_graph_to_domain(py_graph: &PyGraph) -> crate::graph::Graph {
    ...
    let nodes = py_graph
        ...
        .map(|n| Node {
            ...
            // The PyNode snapshot doesn't currently carry the AST / scope
            // overlays back across the boundary. Builders populate them on
            // the Rust side; reconstituting from a PyGraph (used only when
            // crossing PyO3 in the test harness) loses both. The matcher
            // tolerates this — affected predicates short-circuit when
            // ast / scope are empty defaults.
            ast: None,
